...
class VOCSegmentation(data.Dataset):
    ...
    _ Segmentation Dataset.
    Args:
        root (string): Root directory of the VOC Dataset.
        image_set (string, optional): Select the image_set to use, ``train``,
            ``trainval`` or ``val``
        is_aug (bool, optional): If you want to use the augmented train set or
            not (default is True)
    ...
    def __init__(self, root, image_set='train', is_aug=True, transform=None):
        ...
        # Remove first slash otherwise the join will start form root
        self.images = [
            (
...
class VOCSegmentationIncremental(data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, labels=None, 
                labels_old=None, idxs_path=None, masking=True, overlap=True,
                data_masking="current", test_on_val=False, **kwargs):
        full_voc = VOCSegmentation(root, 'train' if train else 'val',
                                    is_aug=True, transform=None)
        ...
        if labels is not None:
            ...
            # take index of images with at least 1 class in labels and 
            # all classes in labels+labels_old+[0, 255]
