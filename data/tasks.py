# This is the original settings of PLOP papers, changes will be executed 
# belongs to the approaches applied
tasks_voc = {
    "offline": {
        0: list(range(21)),
    },
...
# need to check the curent number of class in CitySpaces Dataset
...
# task ADE settings now unchanges
tasks_ade = {
    "offline": {
...
def get_task_list():
    return list(task_voc.keys()) + list(tasks_ade.keys()) \
            + list(tasks_cityscapes_domain.keys())


def get_task_labels(dataset, name, step):
    if dataset == 'voc':
        task_dict = task_voc[name]
    elif dataset == 'ade':
        task_dict = tasks_ade[name]
    elif dataset == 'cityspaces_domain':
        task_dict = tasks_cityscapes_domain[name]
    else:
        raise NotImplementedError
    assert step in task_dict.keys(), f"Need a valid step![{step} out of range]"
