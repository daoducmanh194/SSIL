...
class PI(Regularizer):
    """Path integral Regularizer
    Args:
        model: model at current step t
        model_old: model at step t-1
        device: used device
        score:
        normalize: whether data normalize or not
    """
...
class RW(Regularizer):
    """Regilarizer
    Args:
        model: current model at step t
        model_old: model at step t-1
        device: used device
        score:
        fisher: Fisher Matrix
        alpha: hyper-parameters of Fisher (EWC)
        iterations (int): number of computed iteration
        normalize: whether tensor uses normalize or not
    """
